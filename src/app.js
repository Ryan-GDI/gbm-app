...
function vDocForm(editId, isQuote) {
  ...
  return `<button class="bbk" data-back><i class="ti ti-arrow-left"></i> Back</button>
    ...
    <form id="docForm" data-quote="${isQuote ? '1' : '0'}">
      ...
      <div class="li" style="font-size:11px;color:var(--text-sec);"><div>Description</div><div>Qty</div><div>Unit price</div><div></div></div>
...
function liRow(it, i) { return `<div class="li" data-row="${i}"><input data-f="desc" placeholder="Item description" value="${esc(it.desc)}"><input data-f="qty" type="number" step="0.01" value="${it.qty}"><input data-f="price" type="number" step="0.01" value="${it.price}"><button type="button" data-rm="${i}"><i class="ti ti-x"></i></button></div>`; }
...
function hookDocForm() {
  ...
  f.onsubmit = async ev => {
    ...
    const items = [...lines.querySelectorAll('.li')].map(l => ({desc: l.querySelector('[data-f=desc]').value, qty: Number(l.querySelector('[data-f=qty]').value)||0, price: Number(l.querySelector('[data-f=price]').value)||0})).filter(it => it.desc && it.qty && it.price);
